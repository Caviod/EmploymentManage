<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ReplyManage.aspx.cs" Inherits="admin_ReplyManage" %>
...
<body>
   ...
   <div class="Body_Title">查看回复</div>
    <div>
        ...
        <ItemTemplate>
            <tr>
                <td align="center"><input type="checkbox" value='<%#Eval("ReplyId") %>' name="CheckMes" /></td>
                <td align="center"><%#BLL.UsersBLL.GetIdByUsers(Convert.ToInt32(Eval("UserId"))).UserName%></td>
                   <td align="center" title='<%#Eval("ReplyContent")%>'><%#Eval("ReplyContent").ToString().Length > 40 ? Eval("ReplyContent").ToString().Substring(0, 40) + "..." : Eval("ReplyContent").ToString()%></td>

                   <td align="center"><%#Eval("ReplyTime")%></td>
                <td align="center"><asp:ImageButton ID="ibDel" runat="server" ToolTip="删除" ImageUrl="image/Delete.gif" OnClick="ibDel_ItemDeleting"
                                    CommandArgument='<%# Eval("ReplyId") %>' OnClientClick="return confirm('确定删除当前项?')" />
                </td>
            </tr>
        </ItemTemplate>
